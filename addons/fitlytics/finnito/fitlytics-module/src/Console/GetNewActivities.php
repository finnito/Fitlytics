...
class GetNewActivities extends Command
{
    protected $name = "strava:get";
    protected $description = "Downloads the latest activities JSON from Strava.";
    ...
    public function handle(ActivityModel $activityModel, ActivityRepositoryInterface $activitiesRepository)
    {
        ...
        // echo "Activities before: " . $before->toISOString() . "\n";

        $page = 1;

        $response = $strava->call("/athlete/activities", [
            "before" => $before->timestamp,
            "after" => $latestActivityTimestamp->timestamp,
            "page" => $page
        ]);
